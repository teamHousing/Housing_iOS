...
struct Detail: Codable {
	...
	let progress: Int?
	let requestedTerm, promiseYear: String?
	let promiseMonth, promiseDay: Int?
	...
	init(from decoder: Decoder) throws {
		...
		promiseYear = (try? values.decode(String.self, forKey: .promiseYear)) ?? ""
		...
		replies = (try? values.decode([DetailStatus].self, forKey: .replies) ?? [DetailStatus.init(userStatus: [0], ownerStatus: [0], id: 0)])
	}
}

struct DetailStatus: Codable {
	let userStatus, ownerStatus: [Int]?
	let id: Int
